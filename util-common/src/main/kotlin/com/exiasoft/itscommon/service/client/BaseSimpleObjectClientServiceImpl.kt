...
abstract class BaseSimpleObjectClientServiceImpl<K, I, T : I>(val appName: String, val baseUri: String, val loadBalancingClient: LoadBalancerClient, val webClientBuilder: WebClient.Builder) : BaseSimpleObjectServiceIntf<K, I> {

    val logger = KotlinLogging.logger {}

    private val typeClass = (javaClass.genericSuperclass as ParameterizedType).actualTypeArguments[2]
    ...
    override fun findAll(authenToken: AuthenticationToken, pageable: Pageable): Mono<Page<I>> {
        ...
        return webClientBuilder.build().get().uri {
            it.scheme(client.uri.scheme).host(client.uri.host).port(client.uri.port).path(baseUri).build()
                ...
                .retrieve()
                .bodyToMono(typeRef)
                .map { PageImpl(it.content.asSequence().map { v -> v as I }.toList(), pageable, it.content.size.toLong()) }
    }
    ...
    override fun findByIdentifier(authenToken: AuthenticationToken, id: K): Mono<I> {
        ...
        return webClientBuilder.build().get().uri {
            ...
            it.build()
                ...
                .bodyToMono(typeRef)
                .flatMap {
                    if (it.content.isEmpty()) Mono.empty() else Mono.just(it.content.first() as I) }
    }
    ...
    override fun findByOid(authenToken: AuthenticationToken, oid: String): Mono<I> {
        ...
        return webClientBuilder.build().get().uri {
            it.scheme(client.uri.scheme).host(client.uri.host).port(client.uri.port).path(path).build()
                ...
                .header(HEADER_AUTHORIZATION, authenToken.toString())
                .retrieve()
                .bodyToMono(typeClass as Class<T>).map { v -> v as I }
    }
    ...
    override fun findByOids(authenToken: AuthenticationToken, oids: Set<String>): Mono<Map<String, I>> {
        ...
        return webClientBuilder.build().post().uri {
            it.scheme(client.uri.scheme).host(client.uri.host).port(client.uri.port).path(baseUri).build()
                ...
                // Quick solution is to set token to header, It is better to add webfilter to store all headers and pass to downstream
                .header(HEADER_AUTHORIZATION, authenToken.toString())
                .body(BodyInserters.fromObject(oids)).retrieve().bodyToMono(typeRef).map { it.mapValues { v -> v.value as I } }
    }
